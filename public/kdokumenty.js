// kDokumenty island: markdown editor + live preview + client-side PDF generation.
// Ported from the legacy kdokumenty.js. PDF assembly (jsPDF, embedded Noto/Roboto
...
function parseMarkdownLineToSpans(line) {
  ...
  while (idx <= line.length) {
    ...
    if (nextIdx > idx) spans.push({ text: line.slice(idx, nextIdx), bold, italic });
    if (!match) break;
...
async function ensureLogoData() {
  if (!__logoPromise) {
    __logoPromise = (async () => {
      const r = await fetch('/assets/kdokumenty.png');
...
function applyFormat(action) {
  switch (action) {
    ...
    case 'quote': wrapSelection('', '', (sel) => sel.split('\n').map((line) => (line ? `> ${line}` : '> ')).join('\n')); break;
    case 'ul': wrapSelection('', '', (sel) => (sel || '').split('\n').map((line) => (line ? `- ${line}` : '- ')).join('\n')); break;
    case 'ol': wrapSelection('', '', (sel) => { const lines = (sel || '').split('\n'); let idx = 1; return lines.map((line) => `${idx++}. ${line || ''}`).join('\n'); }); break;
    case 'h2': wrapSelection('', '', (sel) => `## ${sel || 'Nagłówek'}`); break;
    case 'link': wrapSelection('[', '](https://)', (sel) => `[${sel || 'tekst'}](https://)`); break;
    default: break;
  }
}
...
function buildKobdocPayload() {
  return { version: KOBDOC_VERSION, exportedAt: new Date().toISOString(), data: collectData() };
}
...
function applyKobdocData(payload) {
  if (!payload || typeof payload !== 'object') throw new Error('Plik ma nieprawidłowy format (brak danych).');
  const version = payload.version;
  const data = payload.data || payload;
  if (!data || typeof data !== 'object') throw new Error('Plik nie zawiera danych dokumentu.');
  if (version && version !== KOBDOC_VERSION) setStatus(`Inna wersja pliku (${version}) niż wspierana (${KOBDOC_VERSION}). Próba importu...`, 'ok');
...
async function generatePdf(data) {
  ...
  if (data.reference) doc.text(`Sygnatura: ${data.reference}`, 14, 38);
...
function buildPartyLines(p) {
  ...
  if (isPerson) {
    return ['Osoba prywatna', p.nick || '—', p.address || '', p.pesel ? `KESEL: ${p.pesel}` : ''].filter(Boolean);
  }
  return [
    'Instytucja / firma',
    p.company || '—',
...
function bindToolbar() {
  document.querySelectorAll('.kdok-toolbar [data-action]').forEach((btn) => {
...
function bindEvents() {
  ...
  importBtn?.addEventListener('click', () => { clearStatus(); importInput?.click(); });
...
function togglePartyFields(prefix) {
  ...
  box.querySelectorAll('[data-entity]').forEach((el) => {
    el.hidden = el.getAttribute('data-entity') !== type;
  });
}

bindEvents();
document.getElementById('kdok-back')?.addEventListener('click', () => { window.location.href = '/'; });
