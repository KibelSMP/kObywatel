// kSeF invoice generator (island). Ported from the legacy ksef.js. The PDF
// assembly (jsPDF + autoTable + embedded Roboto and Libre Barcode 128 fonts) is
// VERBATIM — only the injected table-row markup and status classes were reskinned
// to Tailwind. Globals: window.jspdf (with autoTable applied), window.escapeHtml.
...
const CELL_INPUT = 'w-full rounded-lg border border-koborder bg-koelev2 px-2 py-1.5 text-sm text-kotext outline-none focus:border-koaccent';
...
function clearStatus() {
  ...
  statusEl.className = 'text-sm text-kodim';
}
function setStatus(msg, kind = 'ok') {
  statusEl.textContent = msg;
  const color = kind === 'err' ? 'text-status-error-text' : 'text-status-success-text';
...
function addRow(prefill = {}) {
  ...
  tr.className = 'border-b border-koborder/60';
  tr.innerHTML = `
    <td class="idx px-2 py-1 text-center text-sm text-kodim"></td>
    <td class="px-1 py-1"><input type="text" class="item-name ${CELL_INPUT}" placeholder="Opis towaru/usługi" value="${prefill.name ?? ''}" /></td>
    <td class="px-1 py-1"><input type="number" class="item-qty ${CELL_INPUT}" min="0" step="0.01" value="${prefill.qty ?? 1}" /></td>
    <td class="px-1 py-1"><input type="text" class="item-unit ${CELL_INPUT}" value="${prefill.unit ?? 'szt.'}" /></td>
    <td class="px-1 py-1"><input type="number" class="item-price ${CELL_INPUT}" min="0" step="0.01" value="${prefill.price ?? 0}" /></td>
    <td class="px-1 py-1"><input type="number" class="item-kat ${CELL_INPUT}" min="0" step="1" value="${prefill.kat ?? 0}" /></td>
    <td class="line-total px-2 py-1 text-right font-mono text-sm text-kotext">0,00</td>
    <td class="px-1 py-1 text-center"><button type="button" class="ksef-remove rounded-lg border border-koborder px-2 py-1 text-xs font-semibold text-status-error-text transition hover:border-status-error" aria-label="Usuń pozycję">Usuń</button></td>`;
...
function buildExportPayload() {
  ...
  return { kind: 'ksef-kob', version: 1, generatedAt: new Date().toISOString(), data };
}
...
function applyImportedData(raw) {
  ...
  if (payload.kind && payload.kind !== 'ksef-kob') throw new Error('Nieprawidłowy typ pliku (kind).');
  const data = payload.data;
  if (!data || typeof data !== 'object') throw new Error('Nie znaleziono danych faktury w pliku.');
  ...
  if (Array.isArray(data.items) && data.items.length) {
    data.items.forEach((item) => {
      addRow({ name: item.name || '', qty: item.qty ?? 1, unit: item.unit || 'szt.', price: item.price ?? 0, kat: item.katRate ?? 0 });
    });
  } else {
    addRow();
  }
...
function handleImportFile(file) {
  ...
  reader.onerror = () => setStatus('Błąd odczytu pliku.', 'err');
...
bindEvents();
document.getElementById('ksef-back')?.addEventListener('click', () => { window.location.href = '/'; });
