// kFirma registration (island). Ported from the legacy kfirma/register.js:
// prefill-from-URL, validation, .kobcomreg generation and the AI-prompt builder
// are unchanged. Overlay/status toggles switch Tailwind `hidden`/`flex` classes
// instead of a legacy `.open` class. Globals: window.__db, window.escapeHtml.
...
const backBtn = document.getElementById('kfreg-back');
...
function setStatus(msg, kind = '') {
  ...
  const color = kind === 'err' ? 'text-status-error-text' : kind === 'ok' ? 'text-status-success-text' : 'text-kodim';
...
function renderBusinessTypeOptions() {
  ...
  BUSINESS_TYPES.forEach((type) => {
    options.push(`<option value="${escapeHtml(type)}">${escapeHtml(BUSINESS_TYPE_LABELS[type] || type)}</option>`);
  });
...
function renderSymbols() {
  ...
  if (!entries.length) {
    symbolsBox.innerHTML = '<div class="text-sm text-kodim">Brak danych o symbolach.</div>';
    return;
  }
  const frag = document.createDocumentFragment();
  entries
    .sort((a, b) => String(a.symbol || '').localeCompare(String(b.symbol || ''), 'pl'))
    .forEach((item) => {
      const sym = String(item.symbol || '').trim();
      if (!sym) return;
      const label = document.createElement('label');
      label.className = 'flex cursor-pointer items-start gap-2 rounded-lg border border-koborder bg-koelev2 px-3 py-2 text-sm hover:border-koaccent/60';
      const input = document.createElement('input');
      input.type = 'checkbox';
      input.value = sym;
      input.name = 'symbols';
      input.className = 'mt-0.5 accent-koaccent';
      const span = document.createElement('span');
      span.innerHTML = `<strong class="text-kotext">${escapeHtml(sym)}</strong> <span class="text-kodim">— ${escapeHtml(item.name || '')}</span>`;
...
function buildPayload(data) {
  return {
    ...
    location: {
      ...
      address: { street: data.street, city: data.city, voivodeship: data.voiv },
    },
  };
}
...
function bindEvents() {
  ...
  aiBtn?.addEventListener('click', () => openOverlay(aiOverlayEl));
  aiCancelBtn?.addEventListener('click', () => closeOverlay(aiOverlayEl));
  aiOverlayEl?.querySelector('.kfreg-overlay-backdrop')?.addEventListener('click', () => closeOverlay(aiOverlayEl));
  aiOpenBtn?.addEventListener('click', () => {
    const url = duckduckgoAiUrl(buildAiPrompt());
    window.open(url, '_blank', 'noopener,noreferrer');
    closeOverlay(aiOverlayEl);
  });
  overlayCloseBtn?.addEventListener('click', () => closeOverlay(overlayEl));
  overlayEl?.querySelector('.kfreg-overlay-backdrop')?.addEventListener('click', () => closeOverlay(overlayEl));
}

prefillData = readPrefill();
renderBusinessTypeOptions();
applyPrefillBasic();
window.__db.loadConfig().then(async () => {
  await loadSymbols();
  bindEvents();
});
